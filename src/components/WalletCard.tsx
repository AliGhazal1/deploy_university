import { RewardWallet } from '../lib/supabaseClient';

interface WalletCardProps {
  wallet: RewardWallet | null;
  loading?: boolean;
}

export default function WalletCard({ wallet, loading }: WalletCardProps) {
  if (loading) {
    return (
      <div className="bg-gradient-to-r from-indigo-500 to-purple-600 text-white p-6 rounded-lg shadow-lg animate-pulse">
        <div className="flex justify-between items-center">
          <div>
            <div className="h-4 bg-white/20 rounded w-24 mb-2"></div>
            <div className="h-8 bg-white/20 rounded w-16"></div>
          </div>
          <div className="text-right">
            <div className="h-4 bg-white/20 rounded w-20 mb-2"></div>
            <div className="h-6 bg-white/20 rounded w-32"></div>
          </div>
        </div>
      </div>
    );
  }

  return (
    <div className="bg-gradient-to-r from-indigo-500 to-purple-600 text-white p-6 rounded-lg shadow-lg">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-lg font-medium opacity-90">Your Balance</h3>
          <p className="text-3xl font-bold">{wallet?.points || 0}</p>
          <p className="text-sm opacity-75">points</p>
        </div>
        <div className="text-right">
          <div className="bg-white/20 rounded-full p-3">
            <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 20 20">
              <path d="M8.433 7.418c.155-.103.346-.196.567-.267v1.698a2.305 2.305 0 01-.567-.267C8.07 8.34 8 8.114 8 8c0-.114.07-.34.433-.582zM11 12.849v-1.698c.22.071.412.164.567.267.364.243.433.468.433.582 0 .114-.07.34-.433.582a2.305 2.305 0 01-.567.267z" />
              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-13a1 1 0 10-2 0v.092a4.535 4.535 0 00-1.676.662C6.602 6.234 6 7.009 6 8c0 .99.602 1.765 1.324 2.246.48.32 1.054.545 1.676.662v1.941c-.391-.127-.68-.317-.843-.504a1 1 0 10-1.51 1.31c.562.649 1.413 1.076 2.353 1.253V15a1 1 0 102 0v-.092a4.535 4.535 0 001.676-.662C13.398 13.766 14 12.991 14 12c0-.99-.602-1.765-1.324-2.246A4.535 4.535 0 0011 9.092V7.151c.391.127.68.317.843.504a1 1 0 101.511-1.31c-.563-.649-1.413-1.076-2.354-1.253V5z" clipRule="evenodd" />
            </svg>
          </div>
          <p className="text-sm opacity-75 mt-2">Campus Rewards</p>
        </div>
      </div>
      {wallet?.updated_at && (
        <p className="text-xs opacity-60 mt-4">
          Last updated: {new Date(wallet.updated_at).toLocaleDateString()}
        </p>
      )}
    </div>
  );
}
